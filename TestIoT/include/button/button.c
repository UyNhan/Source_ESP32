...
typedef struct {
  ...
  uint32_t error_down_time;
  uint32_t error_next_long_time;
} debounce_t;
    ...
    for (;;) {
        for (int idx=0; idx<pin_count; idx++) {
            ...
            if (button_up(&debounce[idx])) {
                ...
                debounce[idx].error_down_time = 0;
                //ESP_LOGI(TAG_BUTTON, "%d UP", debounce[idx].pin);
                send_event(debounce[idx], BUTTON_UP);
            } 
            // else if (debounce[idx].down_time && millis() >= debounce[idx].next_long_time) {
            //     //ESP_LOGI(TAG_BUTTON, "%d LONG", debounce[idx].pin);
            ...
            // } else if (debounce[idx].error_down_time && millis() >= debounce[idx].error_next_long_time) {
            // //ESP_LOGI(TAG_BUTTON, "%d LONG", debounce[idx].pin);
            // debounce[idx].error_next_long_time = debounce[idx].error_next_long_time + CONFIG_ESP32_BUTTON_LONG_PRESS_REPEAT_MS;
            // send_event(debounce[idx], BUTTON_ERROR_CHECKING);                                                                  
            // } 
            else if (button_down(&debounce[idx]) && debounce[idx].down_time == 0) {
                debounce[idx].down_time = millis();
                debounce[idx].error_down_time = millis();
                //ESP_LOGI(TAG_BUTTON, "%d DOWN", debounce[idx].pin);
                debounce[idx].next_long_time = debounce[idx].down_time + CONFIG_ESP32_BUTTON_LONG_PRESS_DURATION_MS;
                debounce[idx].error_next_long_time = debounce[idx].error_down_time + CONFIG_ESP32_BUTTON_ERROR_CHECKING_DURATION_MS;
...
{
    if (pin_count != -1) {
        ESP_LOGI(TAG_BUTTON, "Already initialized");
        return NULL;
    }
    ...
    for (int pin=0; pin<=39; pin++) {
        if ((1ULL<<pin) & pin_select) {
            ESP_LOGI(TAG_BUTTON, "Registering button input: %d", pin);
